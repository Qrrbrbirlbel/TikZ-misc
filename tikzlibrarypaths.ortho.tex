...
%  hvvh/from center/.code={\PackageError{tikz}{The 'from center' style is currently not supported.}},
  hvvh/spacing/.code=%
    \ifcase#1\relax
...
%\def\qtt@tikz@do@hvvh#1#2#3

\let\tikz@hvvh@options\pgfutil@empty
\def\tikz@collect@hvvh@options#1[#2]{\def\tikz@hvvh@options{#2}#1}
%\def\tikz@hvvh@options@{%
%  \begingroup
%    \pgfqkeys{/tikz/hvvh}{/tikz/style/.expanded={\tikz@hvvh@options}}%
%    \xdef\pgf@marshal{\noexpand\def\noexpand\qrr@tikz@hvvh@ratio{\qrr@tikz@hvvh@ratio}%
%                   \noexpand\qrr@tikz@hvvh@distance\the\qrr@tikz@hvvh@distance}
%  \endgroup
%  \pgf@marshal
%}
...
\def\qrr@tikz@hvh@lineto{%
  ...
  {\pgfutil@ifnextchar c{\tikz@collect@coordinate@onpath\qrr@tikz@hvh@lineto}%
    {\pgfutil@ifnextchar[{\tikz@collect@hvvh@options\qrr@tikz@hvh@lineto}
      {\tikz@scan@one@point{\qrr@tikz@@hvh@lineto}}}}}

\def\qrr@tikz@@hvh@lineto#1{%
  \edef\tikz@timer@start{\noexpand\pgfqpoint{\the\tikz@lastx}{\the\tikz@lasty}}%
  \pgf@xc=\tikz@lastx
  \pgf@xb=\tikz@lastx % for "from center"
  \pgf@yc=\tikz@lasty
  ...
  \iftikz@shapeborder%
    {%
      ...
      \xdef\tikz@timer@middle@temp{\noexpand\pgfqpoint{\the\pgf@xa}{\the\tikz@lasty}}%
...
\def\qrr@tikz@vhv@lineto{%
  ...
  {\pgfutil@ifnextchar c{\tikz@collect@coordinate@onpath\qrr@tikz@vhv@lineto}%
    {\pgfutil@ifnextchar[{\tikz@collect@hvvh@options\qrr@tikz@vhv@lineto}
    {\tikz@scan@one@point{\qrr@tikz@@vhv@lineto}}}}}

\def\qrr@tikz@@vhv@lineto#1{%
  \edef\tikz@timer@start{\noexpand\pgfqpoint{\the\tikz@lastx}{\the\tikz@lasty}}%
  \pgf@xc=\tikz@lastx
  \pgf@yb=\tikz@lasty % for "from center"
  \pgf@yc=\tikz@lasty
  \tikz@make@last@position{#1}%
  ...
  \iftikz@shapeborder%
    ...
    \begingroup
      ...
      \pgfutil@tempdimb=\tikz@lasty % for "from center"
      \tikz@make@last@position{\pgfqpoint{\pgf@x}{\pgf@y}}%
      \pgf@ya\pgf@yc
      \ifx\qrr@tikz@hvvh@ratio\pgfutil@empty
        \ifdim\pgf@ya<\pgf@y
      ...
      \tikz@path@lineto{\tikz@last@position}%
      \xdef\tikz@timer@middle@temp{\noexpand\pgfqpoint{\the\tikz@lastx}{\the\pgf@ya}}%
      \xdef\tikz@timer@end@temp{\noexpand\pgfqpoint{\the\tikz@lastx}{\the\tikz@lasty}}%
    \endgroup
    \let\tikz@timer@middle=\tikz@timer@middle@temp
    \let\tikz@timer@end=\tikz@timer@end@temp%
  ...
  \fi%
  \let\tikz@timer=\tikz@timer@vhvline%
  \tikz@scan@next@command%
}
%END_FOLD
...
\def\qrr@tikz@ud@lineto@next{%
  ...
  {\pgfutil@ifnextchar c{\tikz@collect@coordinate@onpath\qrr@tikz@ud@lineto@next}%
    {\pgfutil@ifnextchar[{\tikz@collect@hvvh@options\qrr@tikz@ud@lineto@next}
...
\def\qrr@tikz@du@lineto@next{%
  ...
  {\pgfutil@ifnextchar c{\tikz@collect@coordinate@onpath\qrr@tikz@du@lineto@next}%
    {\pgfutil@ifnextchar[{\tikz@collect@hvvh@options\qrr@tikz@du@lineto@next}
...
\def\qrr@tikz@lr@lineto@next{%
  ...
  {\pgfutil@ifnextchar c{\tikz@collect@coordinate@onpath\qrr@tikz@lr@lineto@next}%
    {\pgfutil@ifnextchar[{\tikz@collect@hvvh@options\qrr@tikz@lr@lineto@next}
